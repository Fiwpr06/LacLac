...
export default function CollectionDetailPage(props: { params: Params }) {
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4">
      {/* Back link */}
      <Link href="/collections" className="inline-flex items-center gap-2 text-brand-primary font-bold mb-6 hover:underline">
      ...
      {/* Collection Banner */}
      {collection && (
        <div className="bg-white rounded-2xl border border-brand-border p-6 md:p-8 mb-8 flex flex-col md:flex-row gap-6 items-center">
          <div className="w-32 h-32 rounded-2xl bg-slate-100 overflow-hidden shrink-0 border border-slate-200">
            {collection.imageUrl ? (
          ...
          <div className="flex-1 text-center md:text-left">
            <h1 className="text-2xl md:text-3xl font-heading font-extrabold text-brand-secondary mb-2">
              {collection.name}
            </h1>
          ...
          <button
            ...
            className="px-5 py-3 bg-brand-primary text-white font-bold rounded-xl hover:bg-brand-primaryHover transition-all shadow-md flex items-center justify-center gap-2 shrink-0 w-full md:w-auto"
          >
      ...
      {/* Foods Grid */}
      <h2 className="text-xl font-bold text-slate-800 mb-6">{t.foodsTitle}</h2>

      {foods.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-card p-12 text-center border border-brand-border">
          <div className="text-5xl mb-4">🍽️</div>
          <p className="text-slate-500 font-medium">{t.noFoods}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {foods.map((food) => (
            <div
              key={food._id}
              className="bg-white rounded-2xl border border-brand-border p-5 hover:shadow-card transition-all flex gap-4"
            >
              {/* Food Image */}
              <div className="w-24 h-24 rounded-xl bg-slate-50 overflow-hidden shrink-0 border border-slate-200 relative">
                {food.imageUrl ? (
                  /* eslint-disable-next-line @next/next/no-img-element */
                  <img src={food.imageUrl} alt={food.name} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full bg-slate-100 flex items-center justify-center text-slate-400 text-2xl">
                    🥗
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start gap-2">
                    <h3 className="text-base font-bold text-slate-800 truncate" title={food.name}>
                      {food.name}
                    </h3>
                    {food.category && (
                  ...
                  {food.note && (
                    <p className="text-xs text-brand-primary italic mt-1.5 truncate">
                      💡 {food.note}
                    </p>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between gap-4 mt-3 pt-3 border-t border-slate-50">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-red-50 hover:bg-red-100 text-red-600 text-xs font-bold rounded"
                    >
                      {t.deleteBtn}
                    </button>
                  </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">{t.labelCategory}</label>
                  <input
                    type="text"
                    value={category}
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">{t.labelSort}</label>
                  <input
                    type="number"
                    value={sortOrder}
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <input
                    ...
                    className="flex-1 px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary"
                    placeholder="http://..."
                  />
                  <label className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl cursor-pointer transition-colors flex items-center justify-center shrink-0">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">{t.labelNote}</label>
                <input
                  type="text"
                  value={note}
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="isRandomEnabled" className="text-sm font-semibold text-slate-700 select-none">
                  {t.labelRandom}
                </label>
              </div>
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-100">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 bg-brand-primary text-white font-bold rounded-xl hover:bg-brand-primaryHover disabled:opacity-50 transition-colors"
                >
